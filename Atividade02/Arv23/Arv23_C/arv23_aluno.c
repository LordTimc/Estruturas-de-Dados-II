...
static Arv23_Aluno *cria_no_ALUNO(ALUNO info, Arv23_Aluno *esq, Arv23_Aluno *cen) {
    Arv23_Aluno *no = (Arv23_Aluno *)malloc(sizeof(Arv23_Aluno));
    if (no) { no->info1 = info; no->nInfo = 1; no->esq = esq; no->cen = cen; no->dir = NULL; }
    return no;
}
static int eh_folha_ALUNO(Arv23_Aluno *no) { return (no != NULL && no->esq == NULL); }

static void adiciona_ALUNO(Arv23_Aluno **no, ALUNO info, Arv23_Aluno *sub) {
    Arv23_Aluno *at = *no;
...
static Arv23_Aluno *quebra_ALUNO(Arv23_Aluno **no, ALUNO info, ALUNO *sobe, Arv23_Aluno *dir) {
...
static int insere_rec_ALUNO(Arv23_Aluno **raiz, ALUNO valor, ALUNO *sobe, Arv23_Aluno **maior) {
