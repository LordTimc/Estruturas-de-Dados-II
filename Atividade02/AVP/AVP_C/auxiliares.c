...
int pega_cod(char *codigo){
    
    char dados_cod[5];
    int valido = 0;

    do {
        printf("Entre com o codigo nesse formato (xx): ");
        int leu = ler_string_info(dados_cod, sizeof(dados_cod));

        if(leu){
            if(num_eh_valido(dados_cod)){
                *codigo = atoi(dados_cod);
                valido = 1;
            }else {
                printf("O codigo tem que possuir 2 digitos numericos.\n");
            }
        } else{
            printf("Leitura do codigo do codigo!\n");
        }
